/**
 * Enhanced wrapper utilities to enable WhiskeySockets/Baileys (v6.7.1+) to send
 * WhatsApp interactive buttons / native flow messages reliably, with image support.
 */
...
function validateSendButtonsPayload(data) {
  ...
  if (!Array.isArray(data.buttons) || data.buttons.length === 0) {
    ...
    data.buttons.forEach((btn, i) => {
      ...
      errors.push(`button[${i}] invalid shape`);
    });
  }
...
async function sendInteractiveMessage(sock, jid, content, options = {}) {
  ...
  if (content.image) {
    ...
    if (prepareWAMessageMedia) {
      const mediaPrepared = await prepareWAMessageMedia({ image: content.image }, { upload: sock.waUploadToServer });
      preparedImage = mediaPrepared.imageMessage;
    } else {
      preparedImage = content.image;
    }
  }

  const contentToConvert = { ...content };
  if (preparedImage) {
    contentToConvert.image = preparedImage;
  }

  const convertedContent = convertToInteractiveMessage(contentToConvert);
  ...
  const candidatePkgs = ['baileys', '@whiskeysockets/baileys'];
  let loaded = false;
  for (const pkg of candidatePkgs) {
    ...
    try {
      ...
      if (generateWAMessageFromContent && normalizeMessageContent && isJidGroup && sock.relayMessage) {
        loaded = true;
      }
    } catch (_) {}
  }
  ...
  if (!loaded) {
    throw new InteractiveValidationError('Missing baileys internals', { context: 'sendInteractiveMessage.dynamicImport' });
  }
...
async function sendInteractiveButtonsBasic(sock, jid, data = {}, options = {}) {
  ...
  const { text = '', footer = '', title, subtitle, image, buttons = [] } = data;
